...
# Your name: Niki Moore
# Your student id: 0926 3893
# Your email: moorenic@umich.edu
...
def make_players_table(data, cur, conn):
    ...
    for player in players:
        id = player['id']
        name = player['name']
        position1 = player['position_id']
        birthyear = player['birthyear']
        nationality = player['nationality']

        query = "SELECT id FROM Positions WHERE position = ?"
        cur.execute(query, (position1,))
        position_id = cur.fetchone()[0]
        query = "INSERT INTO Players (id, name, position_id1, birthyear, nationality) VALUES (?, ?, ?, ?, ?)"
        cur.execute(query, (id, name, position_id, birthyear, nationality))
...
def nationality_search(countries, cur, conn):
    ...
    query = "SELECT name, position_id, nationality FROM Players WHERE nationality IN (" + ",".join(["?" for i in range(len(countries))])
    cur.execute(query, tuple(countries))
    nationality1 = cur.fetchall()
    return nationality1
...
def birthyear_nationality_search(age, country, cur, conn):
    year = 2023 - age
    cur.execute("SELECT name, nationality, birthyear FROM Players WHERE nationality = ? AND birtheyar < ?", (country, year))
    year1 = cur.fetchall()
    return year1
